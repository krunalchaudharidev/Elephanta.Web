...
export default function EditCategoryModal({ open, onClose, category, onUpdated }) {
  ...
  async function handleSubmit(e) {
    ...
    if (!category || !(category.id || category.Id)) {
      setApiError('Category id missing')
      return
    }
    setSubmitting(true)
    try {
      ...
      if (success) {
        onUpdated && onUpdated(res?.id ?? res?.Id)
        onClose()
      } else {
        setApiError(res?.message || res?.Message || 'Failed to update')
      }
    } catch (err) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <form onSubmit={handleSubmit} className="relative z-10 w-full max-w-2xl bg-white rounded-lg shadow-lg p-6 max-h-[90vh] overflow-auto">
          ...
          <div>
            ...
            <select value={parentCategoryId} onChange={(e) => setParentCategoryId(e.target.value)} className="mt-1 block w-full border border-gray-300 rounded px-3 py-2">
              ...
              {parents.filter((p) => {
                ...
                const pid = p.id ?? p.Id
                const cur = category.id ?? category.Id
                return pid !== cur
              }).map((p) => (
        ...
        <div className="mt-6 flex items-center justify-end gap-3">
          ...
          <button type="submit" disabled={submitting} className="px-4 py-2 rounded bg-indigo-600 text-white">{submitting ? 'Saving...' : 'Update'}</button>
        </div>
      </form>
    </div>
  )
